			...
			if category_counter:
				...
				for category_name, category_count in sorted(category_counter.items()):
					...
					for elem1 in filtered_elements_1:
						try:
							family_n_t = elem1.get_Parameter(DB.BuiltInParameter.ELEM_FAMILY_AND_TYPE_PARAM).AsValueString()
							lps_cat    = elem1.get_Parameter(DB.BuiltInParameter.ELEM_CATEGORY_PARAM).AsValueString()
							if elem1.Category.Name not in ["Lines", "Parts", "Scope Boxes"]:
								family_types_counter[family_n_t] += 1
							else: 
								family_types_counter[lps_cat] += 1
						except Exception as e:
							error_message_1 = output.print_md("<pre>      &#9675; <span style='color: black; font-family: Arial; font-size: 13.5px;'> Element Id: '{}' could not be printed due to the following error: '{}'</span><pre>".format(elem1.Id, type(e).__name__))
					...
					for elem2 in filtered_elements_2:
						try:
							...
							error_message_2 = output.print_md("<pre>      &#9675; <span style='color: black; font-family: Arial; font-size: 13.5px;'> Element Id: '{}' could not be printed due to the following error: '{}'</span><pre>".format(elem2.Id, type(e).__name__))
					...
					for elem3 in filtered_elements_3:
						try:
							...
							error_message_3 = output.print_md("<pre>      &#9675; <span style='color: black; font-family: Arial; font-size: 13.5px;'> Element Id: {} could not be printed due to the following error: '{}'</span><pre>".format(elem3.Id, type(e).__name__))
					...
					for elem4 in filtered_elements_4:
						try:
							...
							error_message_4 = output.print_md("<pre>      &#9675; <span style='color: black; font-family: Arial; font-size: 13.5px;'> Element Id: {} could not be printed due to the following error: '{}'</span><pre>".format(elem4.Id, type(e).__name__))
					...
					for elem5 in filtered_elements_5:
						try:
							...
							error_message_5 = output.print_md("<pre>      &#9675; <span style='color: black; font-family: Arial; font-size: 13.5px;'> Element Id: {} could not be printed due to the following error: '{}'</span><pre>".format(elem5.Id, type(e).__name__))
